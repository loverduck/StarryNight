...
public class UpgradeManager : MonoBehaviour {
    ...
    void Update()
    {
        ...
        {
            if (DataController.GetInstance().GetGold() < upgradeDic.FindUpgrade(50001).cost[currentInvenLv])
            {
                invenUpBtn.enabled = false;
                invenUnlock.SetActive(true);
            }
        ...
        {
            if (DataController.GetInstance().GetGold() < upgradeDic.FindUpgrade(50002).cost[currentPerClickLv])
            {
                energyPerClickUpBtn.enabled = false;
                perClickUnlock.SetActive(true);
            }
    ...
    {
        // 골드 빼고
        DataController.GetInstance().SubGold(upgradeDic.FindUpgrade(50001).cost[currentInvenLv]);
        // 공간 늘려주고

...
GUI_ENABLED = true;
		...
		////			ISD_Settings.GetInstance.IscompilerSettingsOpen = EditorGUILayout.Foldout(ISD_Settings.GetInstance.IscompilerSettingsOpen, "Compiler Flags");
		////			
		////			if(ISD_Settings.GetInstance.IscompilerSettingsOpen) {
		//				if (ISD_Settings.GetInstance.compileFlags.Count == 0) {
		//					EditorGUILayout.HelpBox("No Compiler Flags added", MessageType.None);
		//				}
		//
		//				foreach(string flasg in ISD_Settings.GetInstance.compileFlags) {
		//					EditorGUILayout.BeginVertical (GUI.skin.box);
		//					
		//					EditorGUILayout.BeginHorizontal();
		...
		//					bool pressed  = GUILayout.Button("x",  EditorStyles.miniButton, GUILayout.Width(20));
		//					if(pressed) {
		//						ISD_Settings.GetInstance.compileFlags.Remove(flasg);
		//						return;
		//					}
		//					EditorGUILayout.EndHorizontal();
		//					EditorGUILayout.EndVertical ();
		...
		//					if(!ISD_Settings.GetInstance.compileFlags.Contains(NewCompilerFlag) && NewCompilerFlag.Length > 0) {
		//						ISD_Settings.GetInstance.compileFlags.Add(NewCompilerFlag);
		...
		public static void LanguageValues ()	{
			//			ISD_Settings.GetInstance.IsLanguageSettingOpen = EditorGUILayout.Foldout(ISD_Settings.GetInstance.IsLanguageSettingOpen, "Languages");
			//
			//			if(ISD_Settings.GetInstance.IsLanguageSettingOpen)	 {
